...
function accessibleDialog(backdrop){
  const dialog=$('[role="dialog"],.modal',backdrop);if(!dialog)return null;
...
function wrapAsyncForms(backdrop){
  $$('form',backdrop).forEach(form=>{
    ...
    form.dataset.systemicAsyncGuard='1';const original=form.onsubmit;
    form.onsubmit=async function(event){
      if(backdrop.dataset.operationRunning==='1'){event?.preventDefault?.();event?.stopImmediatePropagation?.();return}
      backdrop.dataset.operationRunning='1';
      try{return await original.call(this,event)}finally{backdrop.dataset.operationRunning='0'}
    };
  });
}
function syncModalStack(){
  ...
  open.forEach(backdrop=>{const isTop=backdrop===top;backdrop.inert=!isTop;backdrop.setAttribute('aria-hidden',String(!isTop))});
  $$('.modal-backdrop').filter(backdrop=>!open.includes(backdrop)).forEach(backdrop=>{backdrop.inert=false;backdrop.setAttribute('aria-hidden','true')});
  document.body.classList.toggle('has-modal-open',Boolean(top));
}
function onVisibilityChange(backdrop){
  const state=modalState.get(backdrop);if(!state)return;const visible=isVisible(backdrop);
  if(visible===state.visible){syncModalStack();return}state.visible=visible;
  if(visible){
    const active=document.activeElement;state.opener=active instanceof HTMLElement&&!backdrop.contains(active)?active:null;
    accessibleDialog(backdrop);wrapAsyncForms(backdrop);syncModalStack();
    requestAnimationFrame(()=>{if(!isVisible(backdrop)||visibleModals().at(-1)!==backdrop)return;const target=focusables(backdrop)[0]||accessibleDialog(backdrop);if(target instanceof HTMLElement){if(!target.matches(FOCUSABLE)&&!target.hasAttribute('tabindex'))target.tabIndex=-1;target.focus({preventScroll:true})}});
  }else{syncModalStack();const opener=state.opener;state.opener=null;if(opener?.isConnected)requestAnimationFrame(()=>opener.focus({preventScroll:true}))}
}
function registerModal(backdrop){
  ...
  const startsVisible=isVisible(backdrop),state={visible:false,opener:null,observer:null};modalState.set(backdrop,state);
  accessibleDialog(backdrop);wrapAsyncForms(backdrop);backdrop.setAttribute('aria-hidden',String(!startsVisible));
  state.observer=new MutationObserver(()=>onVisibilityChange(backdrop));state.observer.observe(backdrop,{attributes:true,attributeFilter:['class','hidden']});
...
document.addEventListener('keydown',event=>{
  if(event.key!=='Tab')return;const top=visibleModals().at(-1);if(!top)return;const items=focusables(top);
  if(!items.length){event.preventDefault();const dialog=accessibleDialog(top);if(dialog instanceof HTMLElement){if(!dialog.hasAttribute('tabindex'))dialog.tabIndex=-1;dialog.focus({preventScroll:true})}return}
