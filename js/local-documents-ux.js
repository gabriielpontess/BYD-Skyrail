...
async function renderLocalDocumentsPage(){
  ...
  try{
    ...
    $('[data-local-approval-status]',page).onchange=event=>{localState.approvalStatus=event.target.value;resetAndRerender()};
    $('[data-local-page-prev]',page)?.addEventListener('click',()=>{localState.page=Math.max(1,localState.page-1);rerender();page.scrollIntoView({block:'start'})});
    $('[data-local-page-next]',page)?.addEventListener('click',()=>{localState.page=Math.min(totalPages,localState.page+1);rerender();page.scrollIntoView({block:'start'})});
