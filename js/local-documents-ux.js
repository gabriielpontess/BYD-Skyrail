...
function importModal(){
  ...
  modal.querySelector('[data-import-start]').onclick=async()=>{
    ...
    finally{button.disabled=false;button.classList.remove('is-loading');button.textContent='Validar e importar'}
  };
  return modal;
}
...
async function renderLocalDocumentsPage(){
  ...
  try{
    ...
    page.innerHTML=`<div class="page-head"><div><h1>Documentos</h1><p>Pesquise e filtre o catálogo armazenado localmente, inclusive sem internet.</p></div></div>
      ...
      ${visible.length?`<div class="doc-table-wrap"><table class="doc-table"><thead><tr><th>Código</th><th>Descrição</th><th>Sistema</th><th>Disciplina</th><th>Tipo</th><th>Revisão</th><th>Status</th></tr></thead><tbody>${visible.map(doc=>`<tr data-open-doc="${esc(doc.id)}" tabindex="0" aria-label="Abrir documento ${esc(doc.code)}"><td><span class="doc-code">${esc(doc.code)}</span></td><td><span class="doc-description">${esc(doc.title)}</span>${doc.description?`<small class="local-doc-description">${esc(doc.description)}</small>`:''}</td><td><span class="system-tag">${esc(systemMap.get(doc.system_id)||doc.system_name||'Sem sistema')}</span></td><td>${esc(doc.discipline||'—')}</td><td>${esc(doc.document_type||'—')}</td><td>Rev. ${esc(doc.revision)}</td><td>${approvalBadge(doc.approval_status||doc.source_status)}</td></tr>`).join('')}</tbody></table></div>
      <div class="mobile-document-list">${visible.map(doc=>`<article class="mobile-doc-card" data-open-doc="${esc(doc.id)}" tabindex="0" aria-label="Abrir documento ${esc(doc.code)}"><div class="mobile-doc-top"><span class="doc-code">${esc(doc.code)}</span><span class="status-badge updated">Rev. ${esc(doc.revision)}</span></div><span class="doc-description">${esc(doc.title)}</span><div class="mobile-doc-meta"><span class="system-tag">${esc(systemMap.get(doc.system_id)||doc.system_name||'Sem sistema')}</span><span>${esc(doc.document_type||doc.discipline||'')}</span></div><div class="mobile-doc-meta">${approvalBadge(doc.approval_status||doc.source_status)}</div><div class="mobile-doc-actions"><span class="btn btn-outline">Abrir</span></div></article>`).join('')}</div>`:`<div class="empty-state"><h3>Nenhum documento encontrado</h3><p>Revise o texto pesquisado ou remova um dos filtros para ampliar os resultados.</p></div>`}`;
